...
phil_train_file = "data/new_philosophy_train.csv"
...
def read_dataset_UK_ethos(abs=True):
    if abs:
        return pd.read_csv(tesi_UK_abs_ethos, delimiter="\t", skiprows=1, names=["id", "title","autore","univ","publisher","anno","abstract","tipo","qname", "qlevel", "identifier", "source", "sxsi", "argomento", "unnamed"])
    else:
        return pd.read_csv(tesi_UK_no_abs_ethos, delimiter="\t", skiprows=1, names=["id","title","autore","univ","publisher","anno","tipo","qname", "qlevel", "identifier", "source", "sxsi", "argomento", "unnamed"])
